...
export default function App() {
  ...
  // Cargar productos y ventas desde Supabase
  useEffect(() => {
    if (!user) return;
    loadProducts();
    loadSales();
  ...
  // Ticket ops
  const addItem = (p) => setTicket(prev => {
  ...
  // Gestión productos con Supabase
  ...
  if (loading) return <div style={{ background: '#111', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#D4A017', fontSize: 20, fontFamily: 'sans-serif' }}>Cargando SandoK TPV...</div>;
    ...
    <div style={{ minHeight: '100vh', background: B.offBlack, color: B.white, fontFamily: "'DM Sans','Helvetica Neue',sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', height: 62, background: B.black, borderBottom: `3px solid ${B.mustard}`, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, color: B.muted, background: B.dark, padding: '5px 10px', borderRadius: 20, marginRight: 4 }}>👤 {user.username}</span>
          <button style={navBtn(view === 'pos')} onClick={() => setView('pos')}>🏠 Venta</button>
          <button style={navBtn(view === 'history')} onClick={() => { setView('history'); loadSales(); }}>
            📋 Historial{todaySales.length > 0 ? ` (${todaySales.length})` : ''}
          </button>
          {user.role === 'admin' && <button style={navBtn(view === 'manage')} onClick={() => setView('manage')}>⚙️ Productos</button>}
              ...
              {todaySales.map(sale => (
                <div key={sale.id} style={{ background: B.black, border: `1px solid ${B.mid}`, borderRadius: 14, padding: 18 }}>
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 10 }}>
                    ...
                    {sale.seller && <span style={{ background: B.dark, borderRadius: 6, padding: '3px 10px', fontSize: 11, color: B.muted }}>👤 {sale.seller}</span>}
                    <span style={{ marginLeft: 'auto', fontSize: 20, fontWeight: 900, color: B.mustard }}>{fmt(sale.total)}</span>
                    <button onClick={() => setPrintSale(sale)} style={{ background: B.mustard, border: 'none', borderRadius: 8, color: B.black, padding: '6px 12px', cursor: 'pointer', fontSize: 12, fontWeight: 800, fontFamily: 'inherit' }}>🖨️</button>
                  </div>
                  ...
                  {sale.change_amount > 0 && <div style={{ fontSize: 12, color: '#8BC34A', marginTop: 8, fontWeight: 600 }}>Cambio: {fmt(sale.change_amount)}</div>}
                </div>
              ))}
            </div>
          )}
              ...
              {ticket.length === 0 ? (
                ...
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 18px', borderBottom: `1px solid ${B.dark}` }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 800, color: B.mustard, minWidth: 58, textAlign: 'right' }}>{fmt(Number(item.price) * item.qty)}</span>
                  </div>
                </div>
              ))}
            </div>
